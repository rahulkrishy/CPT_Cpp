// std::stack
/*
-std::stack is a container adaptor provided by the C++ Standard Library that implements the stack data structure. 
-It follows the Last In, First Out (LIFO) principle, meaning that the most recently inserted element is the first one to be removed.
-The std::stack container adaptor is defined in the <stack> header and is typically implemented using an underlying container, such as std::deque, std::vector, or std::list.
-std::vector, std::list , std::deque are possible options for the underlying container, with std::deque being the default one.
-To implement LIFO semantics, a stack requires the underlying container to support back(), push_back(), and pop_back()
-We access elements only from the top with the top() method. 
-We take things out of the stack through the pop() method. It should be noted that pop() doesn't return you anything. 
 It just takes the top element off the stack and the element below it becomes the new top element.
-This can happen until the stack becomes empty.
-std::stack has a size in the collection.
*/
...
template<typename T, typename Container = std::deque<T>> //for the underlying container, with std::deque being the default one, so to take up different containers, use this template
void print_stack(std::stack<T,Container> stack)
{

//template <typename T>
//void print_stack(std::stack<T> stack)   //here stack is passed as a copy, so it won't affect the original stack
//{
...
//template <typename T>
//void clear_stack(std::stack<T>& stack)     //here stack is passed as a reference, so it will affect the original stack
// {
...
int main(){
    ...
    std::cout << " numbers3 : ";
    print_stack(numbers3); // if template is not handled correctly, will get Compiler error 

    std::cout << " numbers2 : ";
    print_stack(numbers2); // if template is not handled correctly, will get Compiler error 
