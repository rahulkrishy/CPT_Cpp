// std::set
/*
-std::set is a container in the C++ Standard Library that represents a sorted set of unique elements. 
-It is implemented as a sorted binary tree, usually a red-black tree, which ensures logarithmic time complexity for most operations such as insertion, deletion, and searching.
-It doesn't matter which order you insert the elements in, the std::set will store them in order.
-This imposes one condition on the elements you can store in the set : they must provide some way to compare elements so that they can be ordered
-The set internally uses operator‹ to order elements, and the types stored in the set must provide operator‹
-Because elements are ordered, finding things in the set is very fast. It's similar to how we find words in the dictionary
-It is possible to customize how the set orders your element either through implementing operator‹ for your types, or by specifying the comparator callback that the set will use to compare the elements.
-The callback can either be : a function pointer, a functor, a lambda function.
- std::set also doesn't allow duplicated elements to be stored in the container, if you try to insert a duplicate, it will just be ignored.
- To decide whether two ele So here are a few facts about std::set not operator==. 
For the Instead, it uses operator‹. Two elements x and y are considered equal
if !(x < Y) && !(У < X).
*/
...
public : 
    ...
    bool operator< (const Book & right_operand)const{
        return this->m_year < right_operand.m_year;    //less than
    }
private : 
    int m_year;
    std::string m_title;
...
int main(){
    ...
    std::cout << " first : " << *result.first << std::endl;  //return value stored in it
    std::cout << " second : " << result.second << std::endl; //return true or false, whether the inseration was success or failure
    ...
    // auto result_emplace = numbers.emplace(13);  // Fails: duplicate
