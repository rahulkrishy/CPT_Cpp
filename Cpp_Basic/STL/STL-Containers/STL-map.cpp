// std::map
/*
-std::map is a container in the C++ Standard Library that represents an associative container of key-value pairs. 
-It is implemented as a sorted binary tree, typically a red-black tree, which ensures logarithmic time complexity for most operations such as insertion, deletion, and searching.
-std::map stores elements ordered by key in increasing order by default
-std::map doesn't store duplicate keys. It can store duplicate values though
-searches for keys is very fast because std::map automatically orders elements as you add them to the collection.
-std::set and std::map share the property that they don't store duplicates, and
that elements are ordered automatically as you add them to the collection
*/
...
template <typename T,typename K>             //2 template parameter(key,value)
